...
This report is about candy preferences based on data from surveying people around Halloween (including both those who go trick-or-treating and those who do not). Note: more info on the data is available [here](https://www.scq.ubc.ca/so-much-candy-data-seriously/). 
...
* 1: [Planning](#1-Plan-for-how-to-prepare-data-for-analysis) how to prepare data for analysis
* 2: [Preparing](#2-Preparing-year-data-for-joining) raw data for joining years to create one dataset
* 3: [Cleaning](#3-Cleaning-data-values-in-joined-data) joined data ready for analysis
...
# 1: Plan for how to prepare data for analysis
...
# 2: Preparing year data for joining
...
# 3: Cleaning data values in joined data
...
Q3: Of the 7,886 raters who did not go out trick-or-treating, the average age was 39 years.
...
candy_ratings_analysis %>% 
  ...
  head(5)
```
Here, we find that **Bonkers** is the specific item with the most MEH ratings, the generic "Any full-sized candy bar" and "Hard Candy" entries.
...
When looking by candy_item_type, M&Ms receive a lot of ratings when combined across all the different types of M&M, and become most-rated in both JOY and MEH.
...
For Qs6-8, count despair as -1, joy as +1, and meh as 0. Here, I assume the average rating value is an indication of overall popularity.
...
I could improve the initial data preparation steps = this currently involves subsetting the raw data using column number, which is the least reproducible and readable way. 

Now that I know the list of candy items that I decide to include in the final dataset (see vector created below), I could instead use this knowledge to filter for only the items in this list, which could then be easily updated (e.g. adding an item to the vector) and/or applied to other years' dataframes. 
