...
x2015_subset_converted_all <- x2015_subset %>% 
  ...
  mutate(age = case_when(
    ...
    id == 5624 ~ "50",
    .default = age)) %>% 
  mutate(age = as.integer(age), # Note: this introduces 74 NAs by coercion
...
x2015_subset_converted_all %>% 
  filter(is.na(age)) # 274 rows, which is 285 (original NAs) minus 11 recoded

# Note: mutate(goes_trick_or_treating = as.logical(goes_trick_or_treating))
# coerces to NA, need different approach to make T/F - not done yet
...
Warning: converting age data form <chr> to numeric coerces some values into NAs. Inspect raw data to understand what I might be missing by doing this.
...
* Recode age values for the IDs as above (excluding 1573, to be removed).
