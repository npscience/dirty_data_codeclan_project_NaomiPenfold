...
x2015_subset_converted_all <- x2015_subset %>% 
  ...
  filter(!id == 20151573) # remove 1 duplicate 
...
colnames_2017 <- as.data.frame(colnames(x2017))
```
...
Follow 2015+2016 cleaning steps, see what needs changing:
...
| ID | Age_raw | Action |
|----|----|----|
| 90278186 | See question 2 | check raw data: "I'd rather not say" - nothing to recode |
...
dim(x2017_subset_converted)
head(x2017_subset_converted)
tail(x2017_subset_converted)
```
...
dim(candy_ratings_allyears) # 753,368 rows x 8 cols
```
...
## Load in from new csv

```{r}
candy_ratings <- read_csv("../clean_data/candy_ratings_allyears.csv")
```
Currently rater info is duplicated many times (once per each candy item rated). First collapse the data to look at rater info only without ratings.
```{r}
...
candy_raters %>% 
  filter(!is.na(age)) %>% 
  filter(age <= 5)
# 0s look real, but babies don't eat candy - so keep but filter out in analyses?
# Here, the 1 also has 1 in country - recode country as NA
# 4 and 5s look real, include

candy_raters %>% 
  filter(!is.na(age)) %>% 
  filter(age == 6)
# all 6s look real, include
```
These all look real, are very few (relative to size of data), and young people can have opinions, so only recode the country == 1 instance. Make a decision about which ages to include in analyses - can filter out if too young.
...
The oldest person that ever lived, as far as is verified so far, was 122 years old (source: [wikipedia](https://en.wikipedia.org/wiki/List_of_the_verified_oldest_people)), so include a cut-off for age no greather than 125. 
...
Instead, for age > 125, recode age as NA.
...
```{r}
candy_ratings %>% 
  filter(str_detect(.$candy_item, "marties"))
```
...
# inspect candy_item_type after cleaning
candy_ratings_after_item_cleaning %>% 
...
Data looks clean, expected categories.
...
Note: the following cleaning steps can be repeated on the full tidy dataset (candy_ratings) because here the country entries are just repeated many times for each individual rating observation - the cleaning steps will catch all the repeats.
...
* Make a new column to use with UK/US/Canada/Other where non-UK/US/Canada/NA are encoded as "other countries")

```{r}
# replace NA ages where numbers are present in country column, before making age a number
candy_raters %>% 
  mutate(country = str_to_upper(country)) %>% 
  filter(str_detect(.$country, "EUA"))
```
...
### Summary from inspecting data:

General:
* filter for age <= 1 & >= 125 when looking at age
* for country, recode any [0-9] as NA_character_ = specifically: id = 20172213 has country = 1

# Analysis

Currently using `candy_ratings_allyears_clean` output from cleaning script

```{r}
# df to analyse for individual ratings
candy_ratings_analysis <- read_csv("../clean_data/candy_ratings_allyears_clean.csv")

# subset of df to use for individual raters
candy_raters_analysis <- candy_ratings_analysis %>% 
  select(id,year,age,gender,country,goes_trick_or_treating) %>% 
  unique()

dim(candy_raters_analysis) # 9348 raters
```
...
(Here, I have not filtered out strange ages, since people can enter incorrect info into the age field while also providing real ratings.)

## Q2+3: Average age by trick-or-treating activity

**REDO: once recoded age from country column (2016 data issue)**
...
Q2: Of the 929 trick-or-treaters who provided valid age data (age 1-125 years), the average age was 35 years.

_Q3. What was the average age of people who are not going trick or treating?_
Q3: Of the 7,877 raters who did not go out trick-or-treating, the average age was 39 years.
...
```{r}
# most JOY rated candy bar
candy_ratings_analysis %>% 
  select(candy_item, candy_item_type, rating) %>% 
  filter(rating == "JOY") %>% 
  #group_by(candy_item) %>% 
  group_by(candy_item_type) %>% 
  summarise(count = n()) %>% 
...
The individual candy item with the most "JOY" ratings was **"Any full-sized candy bar" (7588 JOY ratings)**. This is generic, so the second-most JOY-rated candy item is also notable: **"Reese's Peanut Butter Cups" (7368 JOY ratings)**. However, when looking by type of candy item, the candy type with the most "JOY" was **"M&M's"** (of any type: blue, independent, etc; **total 20,257 JOY ratings**).

```{r}
# most MEH rated candy bar
candy_ratings_analysis %>% 
  select(candy_item, candy_item_type, rating) %>% 
  filter(rating == "MEH") %>% 
  #group_by(candy_item) %>% 
...
The individual candy item with the most "MEH" ratings was **"Lollipops" (1570 MEH ratings)**. This is generic, and in second place, is "Hard Candy" (also generic). The first specific candy item is in third place, **"Bonkers (the candy)" (1488 MEH ratings)**.

However, again when looking by type of candy item, the results are different: the candy type with the most "MEH" was again **"M&M's"** (of any type: blue, independent, etc; **total 5,409 MEH ratings**). The top 4 MEH-rated candy items here are all types of candy that I grouped earlier (M&Ms, Jolly Rancher, Smarties, Licorice).

```{r}
# most DESPAIR rated candy bar
candy_ratings_analysis %>% 
  select(candy_item, candy_item_type, rating) %>% 
...
The individual candy item with the most "DESPAIR" ratings was **"Mary Janes" (10,676 DESPAIR ratings)**. This result is the same whether looking at individual candy items or type of candy items.
...
For Qs6-8, count despair as -1, joy as +1, and meh as 0. Here, I assign values to represent the assumption that JOY is a positive rating of popularity, DESPAIR detracts, and MEH is neutral, and assume the average rating value is an indication of overall popularity.
...
Here, I group "I'd rather not say" and NA together as an "unknown" category of gender.
```{r}
candy_ratings_analysis_num %>% 
  ## Toggle filter to see popularity of candy items for each gender (including unknown / NA):
  #filter(gender == "Female") %>% 
  #filter(gender == "Male") %>% 
  filter(gender %in% c("I'd rather not say", NA_character_)) %>% 
  ## Toggle group_by to see popularity of individual candy items or types of candy item
  #group_by(candy_item) %>%
  group_by(candy_item_type) %>%
...
| Gender | Item/type | Most popular | Popularity |
|----|----|-----|----|
| Female | Either | (1) Any full-sized candy bar | (1) 0.859 |
| "" | "" | (2) Reese’s Peanut Butter Cups | (1) 0.754 |
| Male | Either | (1) Any full-sized candy bar | (1) 0.850 |
| "" | "" | (2) Reese’s Peanut Butter Cups | (1) 0.781 | 
| Unknown | Either | (1) Any full-sized candy bar | (1) 0.858 |
| "" | "" | (2) Reese’s Peanut Butter Cups | (1) 0.812 |

It doesn't matter whether I analyse by individual candy items or types of candy item, the results are the same. Furthermore, the same candy items come top in any gender category: the generic **Any full-sized candy bar** is the most popular, and second place is the specific item: **Reese's Peanut Butter Cups**.
...
candy_ratings_analysis_num %>% 
  filter(gender %in% c("I'd rather not say", NA_character_)) %>% 
  ## Toggle group_by to see results by year
  #filter(year == 2015) %>% 
  #filter(year == 2016) %>% 
  filter(year == 2017) %>% 
  ## Toggle group_by to see popularity of individual candy items or types of candy item
...
* **2015:** (1) **Any full-sized candy bar** (popularity = 0.860); (2) **Reese's Peanut Butter Cups** (popularity = 0.814)
* **2016:** (1) **Any full-sized candy bar** (popularity = 0.714); (2) **Lindt Truffle (popularity = 0.667)**
* **2017: (1) Kit Kat (popularity = 0.829)**

(Again, where a generic item has come top, I've noted the first specific item too. And again, the results are the same whether looking at individual candy items or by candy types.)
